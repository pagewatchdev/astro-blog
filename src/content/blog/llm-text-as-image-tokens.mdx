...
For the prompt I'm asking ChatGPT to pick out and summarize the information from [this post by Karpathy on digital hygiene](https://karpathy.bearblog.dev/digital-hygiene/). It should be something LLM's are good at, but there is also a clear correct answer so its easy to check.
...
Running the prompt with a few different models, we can see there is indeed a significant saving in prompt tokens.
