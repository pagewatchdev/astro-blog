...
When creating the image we need to be careful to get the dimensions right, otherwise OpenAI will resize the image and it wont be able to make out any text.
...
This worked just fine, and the output was similar to using the text-based prompts.
...
There is no real difference with gpt-5-chat, but all the **other models use significantly more completion tokens with the image inputs**. My guess is the difference is due to reasoning tokens that might work differently with image inputs.
